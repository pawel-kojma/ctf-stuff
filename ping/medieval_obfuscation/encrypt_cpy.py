#!/usr/bin/env python3
def return_value(arg1):
	MjwTIhbiOLDowBAf = arg1*0xe5 + 0xF7
...
def obg(arg1, arg2, arg3, arg4, arg5, arg6):
	return (int((str(return_value(arg2))+str(return_value(arg3)))))*return_value(arg1)/(int(str(return_value(arg4)+return_value(arg5))+str(return_value(arg6))))
...
with open("encoded.txt", "rb") as file_handle_read:
	file_contents = b"".join(file_handle_read.readlines())
	with open("flag.txt", "wb") as file_handle_write:
		data = b""
		for i in range(len(file_contents)):
			data += bytes([int(file_contents[i])^return_value(i)])
		file_handle_write.write(data[::-1])