...
with open("encoded", "rb") as read_data_handle:
	data_read = b"".join(read_data_handle.readlines())
	with open("flag.cps", "wb") as write_data_handle:
		data_write = b""
		for i in range(len(data_read)):
			data_write += bytes([int(data_read[i])^RiWdagGgRfVvwHIJ(i)])
		write_data_handle.write(data_write[::-1])