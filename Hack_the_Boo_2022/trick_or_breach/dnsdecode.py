...
def decodeFlags(data, fp):
    ...
    if operator.and_(blah, FLAG_STREAM) != 0 :
        fp['stream'] = ""

    if operator.and_(blah, FLAG_SYN) != 0 :
        fp['syn'] = ""

    if operator.and_(blah, FLAG_ACK) != 0 :
        fp['ack'] = ""

    if operator.and_(blah, FLAG_RST) != 0 :
        fp['rst'] = ""

    if operator.and_(blah, FLAG_HEX) != 0 :
        fp['hex'] = ""

    if operator.and_(blah, FLAG_SESSION) != 0 :
        fp['session'] = ""

    if operator.and_(blah, FLAG_IDENTIFIER) != 0 :
        fp['identifier'] = ""
